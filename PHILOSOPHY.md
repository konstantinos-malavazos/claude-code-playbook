...
- **It's file-blind.** To find one method it reads whole files, which burns tokens and
  fills its context with noise. It also *guesses* symbol names. → Give it **eyes**: a
...
forever. The value shows up on ticket *N+1*, not ticket 1. That is exactly why it's
...
- **AI-infra files are sorted, not blanket-blocked.** The test is *provenance*, not path:
  **if you cloned this repo fresh on a new laptop, would you need this file?** The repo's
  ...
  producing files at those locations. That is exactly what happened.
...
> the driver does **not** own the code or the projects. That is why they are so cautious.
> Every rule above protects something specific — usually *"someone else owns this"* or
...
> **This is the one section that does not survive solo work. It is replaced rather than
> relaxed.** A weekend project has no staging tier, so the environment axis has nothing to
> point at. But a live payment API and your only copy of real data are still there. The
...
1. **Think before coding.** State assumptions. If multiple interpretations exist,
   surface them and don't silently pick. If a simpler approach exists, say so.
...
eval, because a memory that is never retrieved fails silently. No amount of
