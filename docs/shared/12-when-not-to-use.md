...
[solo front-end](../solo/01-the-solo-path.md) specifically — four stages that can cost
...
free — somebody else already decided what to build — and there are four situations where
it buys nothing.
...
spent deciding, and at this size nothing does. Cutting has nothing to cut — the whole
thing is one work unit.
...
the effort, which is not a code question, and charting takes the destination as an *input*
