...
| **Python** | **every hook parses its payload with it**, and most of the rest of the MCP servers are Python processes | on `PATH` for the shell the hooks run in, reachable as `python3` or `python`. The hooks take both, in a preferred order — a Microsoft Store app-execution alias under `AppData/Local/Microsoft/WindowsApps` goes **last**, and is never dropped, so a machine where the alias is the only python still works. The hooks need only the standard library, so any Python 3 will do. **An MCP server may want 3.11+**, so check the ones you pick. With no working Python — none on `PATH`, or nothing there that is actually python — the four blocking hooks exit `2` and **block**, naming each interpreter they tried by resolved path. That is loud and safe rather than silent; see [`templates/hooks/README.md`](../../templates/hooks/README.md) |
