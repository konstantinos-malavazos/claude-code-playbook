...
| **`/start-massive`** | a **foggy** ticket on an existing codebase → a local map of small tickets, then stops | `/charting` pointed at a mature multi-repo workspace instead of a greenfield repo. Use when you sit down to plan and cannot. [13](13-massive-tickets.md) |
| **`/resume-massive`** | walks that map — one ticket per session: claim, dispatch by type, gist, close, regenerate. Owns the three endings and the closing review sequence | a map runs for weeks; every session has to start from the map rather than from memory. [13](13-massive-tickets.md) |
| **`/build-chart-ticket`** | one `make:<layer>` ticket off a map → gatherer delta → planner → the single layer specialist the label names → review, but only once that repo has no other makes left | the map already decomposed the work, so this deliberately has **no** track allocation and no slice fork. [13](13-massive-tickets.md) |
