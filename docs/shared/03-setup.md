...
> `CLAUDE.md` is yours now, the repo's is not — there is no repo yet, and
> [the bootstrap](../solo/04-the-bootstrap.md) writes it. There is no workspace file to
> write at all on a one-repo project. **Step 6** (the
