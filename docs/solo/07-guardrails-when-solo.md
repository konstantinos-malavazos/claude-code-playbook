...
This doc is that re-derivation. Three of the four guardrails hold with their **reason**
rewritten rather than their rule relaxed, one is replaced outright, and the one that
loosens creates the need for a new one that the agile path never needed.
...
along.** That one sentence produces every verdict below with no special-casing — which is
the test that it is the right finding rather than a convenient one.
...
| # | Guardrail | Verdict |
|---|---|---|
| 1 | **You push; the agent doesn't** | **Holds**, reason rewritten — and becomes a per-repo answer. |
...
is the boundary and push leaves it. That rule was already found to be the right one for a
...
mechanism below.** The obvious move — *these are my repos, delete the hook* — fails on a
...
writing anywhere other people can see it* — **audience, not tracker**. Private solo repo:
write freely. **Public** repo: ask, because "nobody is watching" stopped being true.
...
principle — which is why §6 is replaced rather than adjusted.
...
actually changed** — and so does the recipe it banks in memory. A test that asserts on a
...
carving out an exception for `CLAUDE.md` would repeat the original mistake one level
down.**
...
per **file**, and the test above is what answers it. The viewer is not part of the
...
independent things stop a file entering git: the hook blocks the *command*, and
`.gitignore` makes the file invisible to it. The dependency viewer's page needs the second
one — the hook stops it being committed, nothing stops it showing as untracked noise — but
a blanket `.claude/` ignore line also hides the specialists that are supposed to be
...
chain is declared and the specialists exist — a real starting point worth having in
...
**It lives in `~/.claude/`, not in the project.** If it lived inside the repo — a marker
file, or the repo's own `.claude/settings.json` — **the agent could write it and unblock
itself**, and §5's headline is *enforced by the harness, not by trust*. A guardrail the
...
| **The agent adding a line is a crossing you see** | The [repo folder is the line](04-the-bootstrap.md#the-repo-folder-is-the-line) and this file sits outside it. The bootstrap **asks**; you answer. |
...
every guardrail on this page is still enforced by the harness, and the only thing that
...
> seam item — it makes nothing true that the seam is testing for. See
...
protection**, and removing it leaves a hole with a concrete shape:
...
Neither is a guardrail, and they are written down as habits **and labelled as such**:
...
- **The path list was the bug, not the paths.** Provenance expressed as location works
  until your own flow starts producing files at those locations — and then it fails in
...
[04-the-bootstrap.md](04-the-bootstrap.md) — that is the stage where you are asked the two
