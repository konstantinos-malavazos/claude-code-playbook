...
You also arrive with a repo that has been sitting empty since the
[kill gate](02-the-kill-gate.md) created it.
...
| **The stack, and the layer chain** | Named **together**, by the tail's first ticket in [charting](03-charting.md#the-tail). The method is in [06-choosing-the-stack.md](06-choosing-the-stack.md). |
...
before it produced.** Step 1 is the exception, placed by a different rule.

| # | Step | Why it is here |
|---|---|---|
| 1 | **Answer the two allowlist questions** — may the agent push here, and is this repo's `CLAUDE.md` its own | The only part of the stage that is **yours**. It goes first so everything after it runs unattended. Step 3's output also cannot be committed until it is answered. |
...
| 3 | **Write the repo's `CLAUDE.md`** — stack, build/test/run commands, the branch this repo ships from, layer chain, Serena verdict, the settled names | Step 5 reads the chain from it. The seam reads the verdict from it. The names are **copied** from the map's `Notes`, which charting pruned at the tail. This is their second and final home, and none is a valid answer. The branch is **detected**, not asked for. This stage runs inside a repo that already exists, so `git symbolic-ref refs/remotes/origin/HEAD` answers it. No extra step, no extra question. |
...
unattended part starts after you have finished being involved. Nothing stops in the middle
to ask. See [07-guardrails-when-solo.md](07-guardrails-when-solo.md) for why the answers
live outside the repo and default to no.
...
*"the area this repo is usually changed in: `<path>`"*. With no folders on disk, that path
is a guess.
...
A **sparse** index on a real stub is not a step to retry. The verdict said *yes*, and the
symbols do not come back. It is the backwards step: the stack cannot be read the way the
...
dispatches its implement step to **layer specialists**. Until now nothing on this path
guaranteed those agent files existed. Every other check could hold, and the pipeline would
have nothing to dispatch to.

Three different things own three different questions. None of them restates another:
...
Step 5 does not reimplement any of that. It **runs `/adapt-to-stack`**. That reads the
`CLAUDE.md` step 3 just wrote, and generates one specialist, its slice-mode variant and one
standards skill per layer into the repo's own `.claude/`. Read
...
What step 6 checks is narrower and easy to get wrong: whether the **one installed adapter
is the right one for this project's tracker**. Home projects on GitHub and work on Jira is
the normal case. The failure is silent.
...
The *why* is the one part that fits nowhere else. It is exactly what `@context-gatherer`
cannot find on its own, because the reasoning lives in a closed ticket on a map it never
reads. See [05-forgetful.md](../shared/05-forgetful.md).
...
This is **blast radius, not audience**. The distinction matters, because the audience rule
returns the wrong answer here. [`PHILOSOPHY.md`](../../PHILOSOPHY.md) §5 says *ask
before writing anywhere other people can see it*. For a solo builder on a private repo,
that means **never ask**. Their machine has no other people on it. It does have other
projects.
...
codebase, not your machine's configuration. That is what lets step 5 run unattended like
every other step.

**Stage 3 ends with one commit, and only on an all-green report.** Something has to commit
the `CLAUDE.md` step 3 writes, and the generated agent files with it. That commit uses
**explicit paths, never `git add .`**, which the hook blocks whatever step 1 answered.
...
The fix is a test, *would a fresh clone need this file?*, rather than another path
exception. Red means the stack or the setup is wrong. On day one there is nothing worth
...
stage 3's to make true. Item 6 is *the backlog exists*. It belongs to stage 4, and it is
the one thing the report cannot speak to.
...
3 is a checklist. That is the same reason the layer chain is named in stage 2 and not here.
...
only one that writes nothing**. It is a single read of the installed tracker adapter, so
...
of what the disk already says, and the two can disagree. Then the copy is what you read,
and the disk is what is true. That is the spine's
...
The skipping is what makes that cheap. A re-run against a finished repo prints the report
you lost.
...
[03-charting.md](03-charting.md#the-tail). The *method* is in
[06-choosing-the-stack.md](06-choosing-the-stack.md): how one candidate stack gets proposed
and killed, and why the chain is one of the three things ticket 1 decides.
...
| **A `CLAUDE.md`** | Stack, commands, the branch this repo ships from, layer chain, Serena verdict, the settled names. The permanent home for all six, and the only `CLAUDE.md` a one-repo project has below the global one. |
...
| **One report** | Seven of the seam's eight items. [Cutting](05-cutting.md) makes the remaining one true: item 6, *the backlog exists*. It does that against a repo that is real, so its tickets can name paths that exist. |
...
- **The order of the eight steps is forced for seven of them**, not stylistic. Each needs
  the one before it, and re-ordering them quietly breaks something two steps later.
...
- **This is where the layer specialists come from.** It is the gap that made the seam
  eight checks instead of seven.
- **The repo folder is a blast-radius boundary.** That is a different rule from the
  audience rule the rest of the playbook uses, and it is the one that is right here.
- **A red check is not automatically a failure of the stack.** The whole point of
