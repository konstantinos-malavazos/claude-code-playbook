...
drives **[stage 2 — charting](03-charting.md)** and nothing else, and it changes exactly two
...
([03-charting.md:157](03-charting.md)), and the only make the path guarantees is the very
last ticket on it. So this mode is not clearing a mechanical backlog while you sleep. There
is no mechanical backlog here yet; the repo is a stub.

**It is answering your open design questions from the record, overnight, and telling you
every one it had to guess at.**

That is a bigger thing to hand over than it sounds, and the whole design below exists to
make it survivable: every answer carries a **basis**, every guess lands in one file you read
in the morning, and the decisions expensive enough to matter stop the walk instead.
...
and the answer is written into the adapter itself: *"Is this a shared place?"*
([github.md:7](../../templates/trackers/github.md),
[local-markdown.md:9](../../templates/trackers/local-markdown.md)). If the answer is yes, the
walk stops before iteration one.

This is [guardrail 2](07-guardrails-when-solo.md) applying unchanged — *ask before writing
where others can see it*, which that doc keeps as-restated rather than relaxing. A night of
unattended comments and closes is the largest single batch of tracker writes this playbook
can produce, and it is the one nobody read first.

**The audience clause is the half that survives here**, and the reversibility clause is not
...
> and that count **lags a close**. Verified: closing a blocker and immediately reading the
...
> seconds later with no intervening write, and the trap's own words are that **"the frontier
...
and it was correct at the same instant the count was wrong. **On GitHub the walk computes its
...
answer would be `guessed` or `yours` — the mode would halt on iteration one, or, worse under
`--very`, would decide your entire product from nothing. Three closed tickets is not a magic
number; it is the smallest record that can ground anything at all.

**Chart the first few decisions yourself.** That is the part where you find out what you
actually think, and it is the part the steward is worst at.
...
layer chain and all eight seam checks are downstream of it, and undoing it is not a ticket,
it is starting again. The [reversibility test](#the-reversibility-test) would catch it
anyway; this rule is here so it does not depend on the steward classifying it correctly.

`/feeling-very-lucky` will take the tail. **That is a machine picking your stack overnight**,
and it is the sharpest example of what that setting is for and what it costs.
...
Not a list of forbidden things — [the path list was the bug, not the
...
applies to a private stub nobody else has. Having paid that once, up front and
mechanically, reversibility is the only clause left to test per decision — exactly as that
doc predicted.
...
sessions — and every one of them looks like progress in a log, because **a loop cannot tell
...
there is now something that counts. This mode is also what creates the need — it removes the
...
branch, no PR, no waiting — [stalling a loop to ask for a push is the one thing a loop
cannot do](07-guardrails-when-solo.md).

### What is actually being pushed

Almost always: **ticket files.** Stage 2 produces decisions, not code
([03-charting.md:157](03-charting.md)), and on
...
> machine overnight is the decision record — and it is the thing a crashed laptop otherwise
...
**The second one is not optional here, and it is the sharpest reason this mode needs it.**
...
second one is missing, and forty iterations of *what happened and why* has nowhere to go.
...
> watching**, and it is derivable from nothing: the sessions are gone. Delete `ledger.md` and
> the information is not recomputed, it is lost. That is the opposite of the test a progress
> file fails.
...
a pack cannot stand in for a product owner who is simply not in the room — that is the
premise [07-guardrails-when-solo.md:3](07-guardrails-when-solo.md) opens with, and it is why
this mode is solo-only and why no new guardrail was needed to keep it off the team path. The
hook layer says what is *possible*; a flow says what it *does*
...
- **A loop adds no failure modes. It removes the human who was absorbing the ones already
  ...
  before believing a count you did not expect"* — an instruction with a reader in it. A
  human shrugs and re-reads. A loop calls it an ending and goes to bed.
