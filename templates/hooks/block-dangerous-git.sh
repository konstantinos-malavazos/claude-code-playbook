...
if [ -n "$args" ]; then
    # Rows include commands nested in $( ), backticks and any multi-word argument (bash -c,
    # ssh, eval), up to MAX_DEPTH; a commit message and the arguments of echo, printf,
    # grep, rg and gh (NO_EXEC) with no pipe after them are not read as commands, but $( )
    # and backticks in them are. Text a shell reads on stdin (a heredoc body fed to bash) is not.
    push_checked=""
