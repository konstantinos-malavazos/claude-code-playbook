...
**A skill never names a tracker.** It states an intent in the vocabulary below. Exactly
...
three different operations. That is exactly the work a verb contract exists to absorb.
...
call on GitHub and one directory read on local. The obvious client-side assembly is
...
question and silently drops the answer. Every *go and see what that ticket decided*
...
claim back**. Treat a lost race as a nuisance to resolve out of band, not a
correctness bug.
...
That third row is the one neither *"it's Jira"* nor *"it's GitHub"* could answer. It is
