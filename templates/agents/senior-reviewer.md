...
description: >-
  ...
  @reviewer after it drafts a provisional verdict; reads all handoffs + reviewer.md,
  walks every downstream consumer (contract/payload coupling, event topics, schema
  collisions, cross-repo symbol refs), and APPENDS findings to reviewer.md. RELEASE MODE:
...
## Ticket mode
1. Read all `<TICKET-ID>` handoffs + `reviewer.md`.
...
3. **Append** your findings (with severities) to `reviewer.md`. Do not overwrite the
