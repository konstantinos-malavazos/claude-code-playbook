...
see the shape of it before you run it. **Two** ideas go into the kill gate; one of them
comes out the other end as a backlog. The idea, the stack and the names are invented —
substitute your own.

Each stage has its own doc and this page repeats none of them. What it shows instead is
the thing no single stage doc can: **what gets carried across each boundary**, and where
the path stops.
...
The class was novelty, so
[hard kill 2](../docs/solo/02-the-kill-gate.md#the-three-hard-kills) is armed — *it already
exists and it is good* — and Q2's answer fires it. That is what *hard* means: nothing
questions 3 to 5 can say gets weighed against it, and neither your enthusiasm nor
`pitch-judge` can argue it back.

**One sitting. No repo, no map, no weekend.** The ideas file gets a `killed` entry naming
the rule and the reason, so in a year you will find out why past-you said no instead of
re-having the idea from scratch.
...
| **Q2 — does it exist?** | Cold subagent. Because the class is execution, it is briefed to bring back *what exists and what is good about it* rather than a verdict: three mature apps, all of which assume you are buying every ingredient. |
...
| **Q4 — the hard part** | Cold subagent. **Matching free-text pantry lines to recipe ingredients** — *passata*, *chopped tomatoes* and *tinned tomatoes* are one thing on the shelf and three strings on the page. Unsolved in general; small in one household. |
| **Q5 — what it costs you never to build it** | A mildly annoying Sunday, forever. This informs and never kills. |

Hard kill 3 — *the hard part is the whole project* — is armed for this class, and it is
**considered and does not fire**: the search already named a cheap way to find out whether
it is tractable here. That is the judgement the gate exists to make, and it is made out
loud.

The agent then writes **the case for killing** — the strongest one it can, for an idea it
is about to approve — **commits to its own verdict in writing**, and only then asks for
yours. Both say build. `pitch-judge` reads the anonymised case file, with the two verdicts
labelled 1 and 2 in random order, and returns **build** as well.
...
Plus **issue #2**, the hard part, typed `research` — the gate phrased it sharply enough to
be a ticket rather than fog, and `research` runs unattended, so charting has work in flight
...
Eight sessions over three weeks, **one ticket each**. The map is issue #1; its tickets are
its children.

| Session | Ticket | What came out |
|---|---|---|
| 1 | **#2 — how do we match a pantry line to an ingredient?** (`research`) | A hand-written **alias** table over the ~60 things this kitchen actually holds covers it. Embeddings were rejected: you cannot evaluate them without a test set nobody is going to build. Fog thins into two new tickets. |
...
| 3 | **#4 — what happens when a pantry line matches nothing?** (`grilling`) | It is shown beside the shopping list and you teach it the alias there. **The app never guesses silently.** The word **alias** is settled here and written into the map's `Notes`. |
| 4 | **#5 — what does the week actually look like?** (`prototype`) | Three rough takes on one screen; you react to them. One page: five rows, shopping list underneath, no calendar. |
| 5–6 | two smaller decisions | Then the map stops growing. The frontier empties **and nothing is left open** — which is what separates *cleared* from [*stalled*](../docs/solo/03-charting.md#three-endings). |
| 7–8 | **the tail** — below | |
...
Two tickets, in order, and neither was takeable while any other one was
([why](../docs/solo/03-charting.md#the-tail)):

**Ticket 1 — name the stack** *(decision)*. One candidate is proposed and three checks try
to kill it: **Python + FastAPI + SQLite, server-rendered HTML**. No product decision rules
it out. Serena applies — mature language server, and once the alias table has three readers
you *will* ask who calls this. You read Python fluently. Nothing kills it, so the session
ends there — [no comparison is run](../docs/solo/06-choosing-the-stack.md). The same ticket
names the **layer chain — `storage → planner → web`** — and answers the third thing it
owns: **do tests gate?** Here, **no** — `pytest` exits **5** on a repo with no tests, which
is neither a pass nor a failure, so gating on it means special-casing a number on day one.
That is a fact about Python, not a preference.

**Ticket 2 — write the bootstrap checks** *(make)*. Written blind, against a repo that does
not exist yet, and it writes down whatever ticket 1 decided:
...
Nothing is in anyone's head. **Serena is not indexed and that is correct** — the repo is
three weeks old and still empty.
...
(**yes**). One line goes into `~/.claude/repo-allowlist`. Everything after this runs
unattended, which is why it is asked first rather than in the middle.

| Step | What it leaves behind |
|---|---|
| 2 — scaffold the stub | FastAPI's skeleton, **plus `storage/`, `planner/`, `web/`** — empty, because step 5 is about to write an agent file that names each path |
...
It **classifies nothing**, and that is deliberate — the classification is the most
expensive judgement on this path and it is yours. Here it is easy, because you can see all
seven at once: **one red is a typo.** The tail wrote `python -m pantry_plan` and the
generator laid the package out as `src/pantry_plan`. You fix the check in place. Not a
backwards step; four reds would have been.

Re-run: **7 of 7**. Then **one commit**, explicit paths — `git add -A` stays blocked
whatever step 1 answered.
...
It reads **the smallest version out of the map's Notes** — the sentence the gate wrote
...
**Unit 4 is real work, and it is flagged**, because it traces to a *decision* (#4, the app
never guesses silently) and not to a phrase in the sentence. Two choices, and silently
...
The [trace check](../docs/solo/05-cutting.md#the-scope-check-is-a-trace-not-a-count) is
what named the ticket. A count would only have told you the number was fine.
...
| 2 | The stub builds and runs | `python -m src.pantry_plan` serves a page. **No test check** — item 2 makes one optional and the tail said this stack does not gate on one |
...
- **Unit 4 was cut even though it was real work** — the trace check names *which* ticket
  drifted, which a ticket count never can.
